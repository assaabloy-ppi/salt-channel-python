...
class TestM1(BaseTest):
    ...
    def test_M1_dumps_case2(self):
        ...
        # creating M1Packet from bytes
        m1b = packets.M1Packet(ServerSigKeyIncluded=1)
        m1b.from_bytes(m1_dump)
        self.assertEquals(m1b.size, 74)
        self.assertEqual(bytes(m1b), m1_dump)
    ...
    def test_M1_dumps_case3(self):
        ...
        m1.data.TicketSize = util.cbytes(bytes([len(ticket_dump)]))
        self.assertEquals(m1.size, 170)
        self.assertEqual(bytes(m1), m1_dump)

        m1b = packets.M1Packet(TicketIncluded=1)
        m1b.from_bytes(m1_dump)
        self.assertEquals(m1b.size, 170)
        self.assertEqual(bytes(m1b), m1_dump)
    ...
    def test_M1_dumps_case4(self):
        ...
        m1b = packets.M1Packet(ServerSigKeyIncluded=1, TicketIncluded=1)
        m1b.from_bytes(m1_dump)
        self.assertEquals(m1b.size, 202)
        self.assertEqual(bytes(m1b), m1_dump)
